...
> [!CAUTION]
> 如果你拉取安装的库内没有包含下图所示的带有cuda开头的库时，请退出安装（输入N）,然后重新检查安装的CUDA版本是否与pytorch有相对应的库

<ImageCard
  image="https://img.picui.cn/free/2025/05/06/681998be6b5d0.png"
  title="正确拉取的库示例"
  description="拉取的库必须包含有cuda开头的库文件，否则安装的默认会为CPU版本而不是GPU版本"
/>

<ImageCard
  image="https://img.picui.cn/free/2025/05/06/6819a38d76381.png"
  title="成功安装的示例"
  description="使用此方式可以以更简便的办法来安装pytorch环境，但是此方案安装的torch版本并不一定会是最新的版本。"
/>
